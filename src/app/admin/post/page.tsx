...
const Page = () => {
    ...
    const [currentJobRole, setCurrentJobRole] = useState<string>("Sales Intern (7k-20k/month stipend for 6 months and a PPO of 6-10 LPA)");

    const [formData, setFormData] = useState({
        companyName: "McKinley Rice",
        companyOverview: "McKinley Rice is conducting a virtual placement drive in our campus.",
        stipend: "7k-20k",
        salary: "6-10",
                        ...
                        <>
                            ...
                            <section className="flex mt-4">
                                ...
                                <section className="w-full flex flex-col gap-4 mt-4">
                                    ...
                                    <div className='w-full flex flex-col gap-1'>
                                        <Label htmlFor='companyName' className='text-lg font-bold'>Overview</Label>
                                        <Textarea placeholder="A quick overview of the company" id='companyOverview' onChange={handleInputChange} value={formData.companyOverview} />
                                    </div>
                                    ...
                                    <div className='w-full flex flex-col gap-1'>
                                        ...
                                        <div className='flex items-center gap-2'>
                                            <Input
                                                ...
                                                className='h-10'
                                            />
                                            <Button variant="default" onClick={handleAddJobRole} className='h-full'>Add</Button>
                                        </div>
                                        <div className='flex items-center justify-start w-full mt-1 h-8'>
                                            <span className='flex items-center justify-center font-bold'>Selected: </span>
                                            <div className='flex items-center gap-1 ml-4'>
                                                {jobRoles.map((role, index) => (
                                                    <div key={index} className='flex items-center justify-between gap-2 bg-blue-200 text-sm rounded-full p-1 pl-2'>
                                                        <span>{role}</span>
                                                        <div className='p-1 flex items-center justify-center aspect-square rounded-full hover:bg-blue-500 cursor-pointer' onClick={() => setJobRoles(jobRoles.filter((_, i) => i !== index))}>
                                                            <X size={12} />
                                                        </div>
                                                    </div>
                                                ))}
