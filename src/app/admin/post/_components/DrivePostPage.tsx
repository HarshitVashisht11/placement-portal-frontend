...

			{
				isLoading ? <LoadingBar /> :
					!isPreviewing ?
									...
									<form onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-col gap-4 mt-4">
										<div className='w-full flex flex-col gap-1'>
											...
											<Combobox
												...
												onChange={(value: string) => { setValue("company_id", value) }}
											/>
										</div>
										...
										<div className='flex flex-col w-full gap-4'>
											<div className='w-full flex flex-col gap-1'>
												...
												<Label htmlFor='jobLocation' className='text-sm font-medium'>Role Title</Label>
												<Input
													type='text'
													id='title'
													...
													onChange={(e) => { handleRoleInfoChange(e) }}
												/>
											</div>

											<div className='flex gap-4'>
												<div className='w-full flex flex-col gap-1'>
													<Label htmlFor='jobLocation' className='text-sm font-medium'>Stipend Low</Label>
													<Input
														type='number'
														placeholder='15000'
														className='h-10'
												...
												<div className='w-full flex flex-col gap-1'>
													<Label htmlFor='jobLocation' className='text-sm font-medium'>Stipend High</Label>
													<Input
														type='number'
														placeholder='20000'
														className='h-10'
												...
												<div className='w-full flex flex-col gap-1'>
													<Label htmlFor='jobLocation' className='text-sm font-medium'>Salary Low</Label>
													<Input
														type='number'
														placeholder='40000'
														className='h-10'
												...
												<div className='w-full flex flex-col gap-1'>
													<Label htmlFor='jobLocation' className='text-sm font-medium'>Salary High</Label>
													<Input
														type='number'
														placeholder='50000'
														className='h-10'
												...
												<div className='w-full flex items-end justify-end mt-2'>
													<Button onClick={() => handleAddRole()} className='flex w-full h-10 items-center justify-center gap-2'>
														<Plus strokeWidth={4} size={16} />
														<span> Add this role</span> </Button>
												</div>
											</div>
													...
													<tbody>
														{jobRoles.length > 0 ? jobRoles.map((role) => (
															<tr key={role.id}>
																...
																<td className="px-4 py-2 text-center">{role.stipend_low}</td>
																<td className="px-4 py-2 text-center">{role.stipend_high}</td>
																<td className="px-4 py-2 text-center">{role.salary_low}</td>
																<td className="px-4 py-2 text-center">{role.salary_high}</td>
																<td className="px-4 py-2 text-center flex gap-2 justify-center">
																	<Button variant={'ghost'} onClick={() => handleDeleteRole(role.id)} className='text-red-500'>
																		<Trash size={16} strokeWidth={3} />
																	</Button>
																</td>
															</tr>
														)) : <tr><td colSpan={6} className="px-4 py-2 text-center border-b"> No Roles added yet!</td></tr>}
													</tbody>
												</table>
											</div>
										</div>
										...
										<div className='w-full flex flex-col gap-1'>
											...
											<QuillRichTE
												id="requirements"
												onChange={(value: string) => handleQuillChange("qualifications", value)}
											/>
										...
										<div className='w-full flex flex-col gap-1 mt-8'>
											...
											<QuillRichTE
												id="pointsToNote"
												onChange={(value: string) => handleQuillChange("points_to_note", value)}
											/>
										</div>
						...
						: <></>
				// <CompanyPreviewPage jobRoles={jobRoles} setPreview={setIsPreviewing} />
			}
